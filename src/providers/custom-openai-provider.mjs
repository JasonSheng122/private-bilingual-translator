...
export const CUSTOM_OPENAI_PROVIDER = Object.freeze({
  id: "custom_openai",
...
export async function translateWithCustomOpenAiProvider(segments, options = {}) {
  ...
  const endpoint = options.endpoint ?? buildCustomOpenAiEndpoint(providerConfig);

  if (!isAllowedCustomOpenAiUrl(endpoint, providerConfig)) {
  ...
  const maxRequestSize = normalizeMaxRequestSize(options.maxRequestSize, CUSTOM_OPENAI_PROVIDER.maxRequestSize);
  const maxConcurrentRequests = normalizeMaxConcurrentRequests(
    options.maxConcurrentRequests,
    getDefaultMaxConcurrentRequests(qualityMode)
  );
  const batches = buildCustomOpenAiRequestBatches(normalizedSegments, { qualityMode, maxRequestSize, providerConfig });
  ...
  return {
    ok: true,
    provider: getCustomOpenAiProviderMetadata(providerConfig),
    translations: batchResult.translations
  };
}

export function getCustomOpenAiProviderMetadata(providerConfig) {
  const normalizedConfig = normalizeCustomProviderConfig(providerConfig);
  const endpoint = normalizedConfig ? buildCustomOpenAiEndpoint(normalizedConfig) : "";
  const origin = normalizedConfig ? new URL(normalizedConfig.baseUrl).origin : "";

  return {
    id: CUSTOM_OPENAI_PROVIDER.id,
    displayName: CUSTOM_OPENAI_PROVIDER.displayName,
    endpoint,
    allowedEndpoints: endpoint ? [endpoint] : [],
    allowedOrigins: origin ? [origin] : [],
    model: normalizedConfig?.model ?? "",
    maxRequestSize: CUSTOM_OPENAI_PROVIDER.maxRequestSize,
    maxConcurrentRequests: CUSTOM_OPENAI_PROVIDER.maxConcurrentRequests,
    requiresKey: CUSTOM_OPENAI_PROVIDER.requiresKey
  };
}

export function isAllowedCustomOpenAiUrl(value, providerConfig) {
  try {
    const endpoint = buildCustomOpenAiEndpoint(providerConfig);
...
export function parseCustomOpenAiResponse(data) {
  return parseGeminiLikeResponse(data);
}

function buildCustomOpenAiRequestBody(segments, { qualityMode, providerConfig, retry = false }) {
  return {
    model: providerConfig.model,
    messages: [
      {
        role: "system",
        content: "You are a private webpage translation engine. Return only valid JSON. Translate source text into Simplified Chinese; do not copy source English prose."
      },
      {
        role: "user",
        content: buildCustomOpenAiPrompt(segments, qualityMode, { retry })
      }
    ],
...
function buildCustomOpenAiRequestBatches(segments, { qualityMode, maxRequestSize, providerConfig }) {
  const batches = [];
  let currentBatch = [];
  const targetBatchRequestSize = getTargetBatchRequestSize(qualityMode, maxRequestSize);

  for (const segment of segments) {
    const singleRequestPair = buildCustomOpenAiRequestPair([segment], { qualityMode, providerConfig });
    ...
    const candidateRequestPair = buildCustomOpenAiRequestPair(candidateBatch, { qualityMode, providerConfig });
    ...
    batches.push({
      ...
      ...buildCustomOpenAiRequestPair(currentBatch, { qualityMode, providerConfig })
    });
...
function buildCustomOpenAiRequestPair(segments, { qualityMode, providerConfig }) {
  return {
    requestJson: JSON.stringify(buildCustomOpenAiRequestBody(segments, { qualityMode, providerConfig })),
    retryRequestJson: JSON.stringify(buildCustomOpenAiRequestBody(segments, { qualityMode, providerConfig, retry: true }))
  };
}
...
function buildCustomOpenAiPrompt(segments, qualityMode, { retry = false } = {}) {
  ...
  return [
    ...
    "Return only JSON: {\"translations\":[{\"id\":\"same id\",\"text\":\"translated text\"}]}",
...
async function runCustomOpenAiBatches(fetchImpl, batches, { endpoint, apiKey, maxConcurrentRequests }) {
  ...
  async function worker() {
    while (!firstError) {
      ...
      const response = await requestCustomOpenAiTranslation(fetchImpl, {
        endpoint,
        apiKey,
      ...
      try {
        const translations = parseCustomOpenAiTranslations(response.text, batch.segments);
...
async function requestCustomOpenAiTranslation(fetchImpl, { endpoint, apiKey, requestJson }) {
  let response;
  ...
  try {
    ...
    return {
      ok: true,
      text: parseCustomOpenAiResponse(data)
    };
  } catch {
...
function parseCustomOpenAiTranslations(text, segments) {
  return parseGeminiLikeTranslations(text, segments, "Custom OpenAI");
}
...
function getDefaultMaxConcurrentRequests(qualityMode) {
  return qualityMode === QUALITY_MODES.DEEP
    ...
    : CUSTOM_OPENAI_PROVIDER.maxConcurrentRequests;
}
...
function getProviderHttpErrorMessage(status) {
  ...
  if (status === 429) {
    return "The translation provider rate limited the request.";
  }
...
function getProviderPayloadErrorMessage(message, status) {
  ...
  if (status === 429 || /\b(rate\s*limit|quota|too\s*many)\b/i.test(message)) {
    return "The translation provider rate limited the request.";
  }
